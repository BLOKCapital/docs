...
description: Connectez-vous avec Google. Créez votre compte de portefeuille intelligent (SWA). C'est le compte qui détiendra vos actifs et interagira avec le protocole.
position: 2
---
![Parcours de l'utilisateur](/img/Userflow.png)
...
Les sections ci-dessus décrivent le parcours dans l'ordre. Les pages suivantes décomposent chaque partie en étapes réelles dans l'application, y compris ce qui se passe en cas d'échec.

- [Connexion et Tableau de Bord](/fr/concepts/user-journey/signing-in) couvre les deux méthodes de connexion, ce que charge le tableau de bord, et la fin des sessions.
- [Créer et Financer des Jardins](/fr/concepts/user-journey/gardens) couvre la création d'un Jardin de rendement ou d'Index, l'écran de détails, et l'entrée des actifs.
- [Trading et Prêt](/fr/concepts/user-journey/trading-and-lending) couvre les swaps, le prêt et le retrait d'USDC.
- [Jardins d'Index](/fr/concepts/user-journey/index-gardens) couvre le parcours des index, la connexion, le rééquilibrage et la déconnexion.
- [Performance, Gouvernance et Paramètres](/fr/concepts/user-journey/performance-and-governance) couvre les graphiques de performance, le vote et la section compte.
- [États de Transaction](/fr/concepts/user-journey/transaction-states) couvre la séquence de statuts partagée par toutes les actions on-chain.
