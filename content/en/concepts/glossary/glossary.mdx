---
title: Glossary
description: Core Architecture (Diamond Pattern / EIP-2535) · Garden & Factory · Index & Rebalancing · Protocol Governance & Status · DEX & Liquidity
position: 1
---
...
| Rebalance Interval | Minimum time (24 hours) between consecutive rebalances. |
| Intent Interval | Minimum time (11 minutes) between rebalance intent creations. Kept longer than the 10-minute intent expiry so a live intent cannot be overwritten. |
| Swap Call | Individual swap instruction supplied by the caller, containing selector, encoded data, output token, minimum output and a deadline. |
