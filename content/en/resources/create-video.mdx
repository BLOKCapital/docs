...
description: 'BLOKC thrives on open contributions, and our documentation is no exception.'
position: 7
---
Whether you're a developer, designer, or community member, your input helps make our docs more helpful and inclusive for everyone.
This guide walks you through the basic setup and knowledge you’ll need to start contributing to the BLOKC documentation hosted on [GitHub](https://github.com/BLOKCapital/docs).
...
2. **Node.js and npm**
Required to run the Next.js documentation site locally.
...
Our documentation is written in .mdx files.
...
2. **Fork the Repository**
   Click **"Fork"** button, GitHub will create a copy of the repo in your account. (e.g., https://github.com/your-username/docs).
...
- Each **folder** under `content/<locale>/` represents a major section of the documentation, one folder per language (`en`, `es`, `fr`).
- Each **file** within those folders is a subsection or content page.
- All files are written in **MDX** (`.mdx`) format.
...
- Open the corresponding `.mdx` file.
...
- Create a new file with an `.mdx` extension (e.g., `new-subsection.mdx`).
- Add your content in Markdown format.

#### 3. Adding a New Section

- Create a new folder under `content/<locale>/` with an appropriate name.
- Inside that folder, create a new `.mdx` file for your content.
...
## Resources
- [Next.js Docs](https://nextjs.org/docs) – How the framework this site is built on works.
